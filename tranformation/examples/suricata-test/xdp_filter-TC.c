...
static int __always_inline filter_ipv4(struct __sk_buff* ctx, void* data, __u64 nh_off, void* data_end, __u16 vlan0, __u16 vlan1)
{
	...
	if (value) {
		...
		if (!iface_peer) {
		  ...
		  //IRL: Should we remove this since we have already gone through network stack processing
	...
	if (cpu_max && *cpu_max) {
		...
		if (!cpu_selected)
		  //IRL: Replace XDP_ABORTED with TC_ACT_SHOT
		  return TC_ACT_SHOT;
...
static int __always_inline filter_ipv6(struct __sk_buff* ctx, void* data, __u64 nh_off, void* data_end, __u16 vlan0, __u16 vlan1)
{
...
SEC("xdp")
int xdp_hashfilter(struct __sk_buff* ctx)
{
	...
	if (g_switch && *g_switch) {
		iface_peer = bpf_map_lookup_elem(&tx_peer_int, &key0);
		if (!iface_peer) {
		  //IRL: Transform XDP_DROP to TC_ACT_SHOT
			return TC_ACT_SHOT;
		} else {
			//return bpf_redirect_map(&tx_peer, tx_port, 0);
		}
	...
	//IRL: eth->h_proto to ctx->protocol
	h_proto = ctx->protocol;

	//IRL: Remove vlan_hdr parsing
	struct vlan_hdr* vhdr;
	h_proto = ctx->vlan_proto;
	//IRL
	...
	if (h_proto == __constant_htons(ETH_P_8021Q) || h_proto == __constant_htons(ETH_P_8021AD)) {
		...
		h_proto = ctx->vlan_proto;
#if VLAN_TRACKING
		//IRL: Replace vlan0 = vhdr->h_vlan_TCI & 0x0fff with sk_buff field
		vlan0 = ctx->vlan_tci & 0x0fff;
#else
		vlan0 = 0;
#endif
	}
	if (h_proto == __constant_htons(ETH_P_8021Q) || h_proto == __constant_htons(ETH_P_8021AD)) {
	  ...
	  //IRL: Transform: Remove vlan_hdr parsing code and use sk_buff field
	        //struct vlan_hdr* vhdr;
	        //vhdr = data + nh_off;
		//nh_off += sizeof(struct vlan_hdr);
		//if (data + nh_off > data_end)
	...
	//IRL: Replace XDP_PASS with TC_ACT_OK
	return TC_ACT_OK;
}
